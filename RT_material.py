...
class Emissive(Material):
    def __init__(self, cColor, fIntensity=1.0):
        super().__init__()
        self.emit_color = cColor
        self.intensity = fIntensity
    ...
    def emitting(self):
        return self.emit_color * self.intensity
...
class Checker(Material):
    ...
    def scattering(self, rRayIn, hHinfo):
        ...
        x = math.floor(hHinfo.point.x()*self.scale)
        z = math.floor(hHinfo.point.z()*self.scale)
        ...
        scattered_dir = uvw.local(rtu.Vec3.random_cosine_hemisphere_on_z())

        scattered_ray = rtr.Ray(hHinfo.getP(), scattered_dir, rRayIn.getTime())

        return rtu.Scatterinfo(scattered_ray, color)
